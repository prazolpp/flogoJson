...
const getHandlerArr = (asyncapi, resourceType) => {
  return asyncapi.channelNames().map(channelName => {
    ...
    return {
        settings: {
            method: `<The method name>`,    //////todo: which function to use? 
            path: channelName
        },
        action: {
            ref : `#${resourceType}`,
...
const getHandlersFromServers = (asyncapi, resourceType) => {
  ...
  return Object.entries(asyncapi.servers()).map(([serverName, serverInfo], index) => {
    ...
    return {
        ...
        settings: {
            port: brokerUrl.replace("{port}", currServer.variable("port").defaultValue()),
        },
...
const getImports = (resourceType) => {
    return [`github.com/project-flogo/${resourceType}`,"github.com/project-flogo/contrib/trigger/rest"] ;
}
